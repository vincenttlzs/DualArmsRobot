...
class Pub3DTarget():
    def __init__(self):
        ...
        dyn_client = dynamic_reconfigure.client.Client("s_o", timeout=60)
        ...
        while not rospy.is_shutdown():
            ...
            if timer == 0.0 or timer > self.r_move_target and theta < 50 * self.speed / self.move_target_rate:
                timer = 0.0
                target.pose.position.x = -0.4 #- 0.1 * abs(math.cos(theta))
                target.pose.position.y = 0.7 - 0.1 * theta #* math.sin(theta)
