...
"""
    s_object.py - Version 1.0 2014-01-02
...
class Pub3DTarget():
    def __init__(self):
        ...
        self.speed = rospy.get_param('~speed', 0.02)
        ...
        dyn_client = dynamic_reconfigure.client.Client("s_object", timeout=60)
        ...
        marker_color = rospy.get_param('~marker_color', {'r': 1.0, 'g': 0.2, 'b': 0.0, 'a': 0.8})
        ...
        target.pose.orientation.y = -0.2
        target.pose.orientation.z = 0
        target.pose.orientation.w = 1

       #scene.add_sphere('marker', target_pose, marker_scale)
        ...
        while not rospy.is_shutdown():
            ...
            if timer == 0.0 or timer > self.r_move_target:
                ...
                target.pose.position.y = -0.2 * math.sin(theta) + 0.3
                target.pose.position.z = 0.4 + 0.3 * abs(math.cos(theta))
